...
- Draw.io: Use organic/radial tree layout with curved connectors
...
## PHASE 3: Calculate Layout (Radial Tree)
Layout algorithm for organic mindmap appearance:
- **Root node position:** Center of canvas (x=400, y=300)
- **Root node size:** Larger than children (width=160, height=80)
- **Level 1 nodes:** Arranged in a circle around root
  - Radius: 200px from root center
  - Evenly distributed by angle: 360° / number_of_children
  - Size: width=140, height=60
- **Level 2 nodes:** Fan out from their Level 1 parent
  - Radius: 150px from parent center
  - Angle range: ±45° from parent's angle from root
  - Size: width=120, height=50
- **Level 3+ nodes:** Continue pattern with decreasing radius (120px) and size
  - Size reduction: width=100, height=40
...
## PHASE 6: Position Calculation Algorithm
...
function calculatePositions(root):
    ...
    for each level1_child at index i:
        angle = (360 / count_of_level1_children) * i
        level1_child.x = root.x + 200 * cos(angle)
        level1_child.y = root.y + 200 * sin(angle)
        level1_child.angle = angle
        
        for each level2_child at index j:
            spread = 90 / (count_of_level2_children + 1)
            child_angle = level1_child.angle - 45 + spread * (j + 1)
            level2_child.x = level1_child.x + 150 * cos(child_angle)
            level2_child.y = level1_child.y + 150 * sin(child_angle)
  ...
  <diagram name="[Mindmap Name]" id="[unique]">
    <mxGraphModel dx="1200" dy="800" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="1200" pageHeight="900" math="0" shadow="0">
      <root>
        <mxCell id="0"/>
        <mxCell id="1" parent="0"/>
        [Root node as central mxCell]
        [Level 1 nodes radiating from root]
        [Level 2+ nodes branching from parents]
...
<layout_tips>
- For mindmaps with many Level 1 nodes (>6), consider using a semicircle layout (180°) instead of full circle
- If nodes have long text, increase node width and adjust radius accordingly
- For very deep hierarchies (4+ levels), consider using a horizontal tree layout instead of radial
- Maintain minimum 30px gap between non-connected nodes to prevent visual clutter
</layout_tips>
