...
### Draw.io Layout Rules (CRITICAL - Prevents Cramped Mindmaps) - UPDATED v6.0.0-alpha.11
...
- Root: 180×90
- Level 1: 160×70
- Level 2: **160×55 for long text (>15 chars), 140×55 for short text** (increased width!)
- Level 3+: **140×45 for long text, 120×45 for short text** (increased from fixed 120×45)
- **TEXT LENGTH RULE:** If label > 20 characters, increase node width by 20px
- **NEVER truncate text** - always size node to fit full label

**Anti-Cramping (CRITICAL - PREVENTS BUNCHING) - v6.0.0-alpha.11:**
- **Level 2 nodes MUST be staggered vertically by ±60px** (increased from ±50px!)
- **Level 3 nodes MUST be staggered vertically by ±70px** (increased from ±60px)
- **Minimum 120px gap** between any non-connected nodes (increased from 100px)
- **Fan angles MUST be wider:**
  - Level 2 fan: **±85°** from parent angle (increased from ±80°)
  - Level 3 fan: **±75°** from parent angle (increased from ±70°)
- **Quadrant-aware positioning:** Nodes on left side of diagram fan LEFT, nodes on right fan RIGHT
- **NEVER place 2+ L2 or L3 nodes in a near-horizontal line** - force vertical stagger (threshold: 2 nodes, not 3!)

**Sibling L2 Collision Prevention (ENHANCED v6.0.0-alpha.11 - CRITICAL):**
- **BEFORE calculating L2 positions:** Apply mandatory vertical stagger to ALL L2 children
- L2 stagger pattern: alternate +60px, -60px, +60px from base radius position (increased from ±50px)
- If two L2 nodes are within 250px of each other (increased from 200px):
...
- **L2 children of same L1 must NEVER share similar Y-coordinates (within 50px)** (tightened from 40px)
- If overlap detected: rotate entire L2 fan by 40° away from collision (increased from 35°)
...
- **Check ALL node pairs, not just parent-child** - L2/L3 nodes from different branches can collide!
- If any two non-connected nodes are within 100px, take corrective action:
  1. First try: Rotate the fan angle of the deeper node's parent by 25°
  2. Second try: Increase radius of the deeper node's parent by 35%
...
**Horizontal Line Prevention (ENHANCED v6.0.0-alpha.10 - APPLIES TO L2 AND L3):**
- After positioning, scan ALL L2 and L3 nodes (not just L3!)
- Group nodes by Y-coordinate (within **40px tolerance** - tightened from 50px)
- **If 2+ nodes share similar Y (within 40px): STAGGER IMMEDIATELY** (threshold reduced from 3 to 2!)
- Stagger amount: alternate ±60px vertically
- **Priority:** Stagger the node that is further from canvas center
...
## PHASE 3: Calculate Layout (Radial Tree) - UPDATED v6.0.0-alpha.11
...
### LEVEL 2 NODES (Sub-branches) - MAXIMUM SPREAD WITH MANDATORY STAGGER
- **Radius:** 280px from parent center (increased from 240px)
- **Angular fan:** Spread evenly within ±80° from parent's angle (increased from ±75°)
- **Size:** width=140, height=55
- **QUADRANT AWARENESS:** Fan direction follows parent's quadrant (see below)
- **MANDATORY VERTICAL STAGGER:** L2 nodes must alternate ±50px from base Y position
...
- **Size:** width=140 for long text (>15 chars), 120 for short text; height=45
- **Font size:** Minimum 11px (never smaller)
- **VERTICAL STAGGER:** Alternate nodes offset by ±70px vertically (increased from ±60px)
- **HORIZONTAL LINE CHECK:** If 2+ nodes (not 3!) have y within ±50px, stagger by 70px

### QUADRANT-AWARE POSITIONING (ENHANCED v6.0.0-alpha.11)
Determine which quadrant the parent is in relative to root, then fan children OUTWARD:
- **TOP quadrant (angles 210°-330°):** Children fan from -85° to +85° ABOVE parent (wider range)
- **BOTTOM quadrant (angles 30°-150°):** Children fan from -85° to +85° BELOW parent (wider range)
- **LEFT quadrant (angles 150°-210°):** Children fan from -85° to +85° to the LEFT
- **RIGHT quadrant (angles 330°-30° / 330°-360° + 0°-30°):** Children fan to the RIGHT

### SIBLING L2 COLLISION PREVENTION (ENHANCED v6.0.0-alpha.11 - CRITICAL)
For L1 nodes with multiple L2 children:
1. **FIRST:** Apply mandatory ±60px vertical stagger to ALL L2 children before further positioning
2. Calculate bounding box for each L2 subtree (L2 node + all its L3 children)
3. Check if adjacent L2 subtree bounding boxes overlap (within 250px)
4. If overlap detected: increase L2 radius by 40% and rotate fan by 30°
5. Re-run collision check until no overlaps

### HORIZONTAL LINE PREVENTION (ENHANCED v6.0.0-alpha.11 - APPLIES TO L2 AND L3)
After positioning ALL L2 and L3 nodes:
1. Group ALL nodes (L2 and L3) by Y-coordinate (within 50px tolerance - tightened!)
2. **If 2+ nodes share similar Y: STAGGER IMMEDIATELY** (threshold is 2, not 3!)
3. Stagger amount: ±70px vertical offset (increased from ±60px)
4. Priority: Stagger the node furthest from canvas center
5. This prevents horizontal bands at ANY level

### COLLISION AVOIDANCE (ENHANCED v6.0.0-alpha.11 - RUN AFTER POSITIONING)
1. After calculating ALL positions, check every node pair
2. Minimum 120px gap between any non-connected nodes (increased from 100px)
3. If nodes overlap: increase parent radius by 40% and recalculate that branch
4. Level 2 AND 3 nodes should never form horizontal lines - stagger vertically
...
## PHASE 6: Position Calculation Algorithm (UPDATED v6.0.0-alpha.11)
...
function calculateCanvasSize(root):
    ...
    // Scale canvas based on content - v6.0.0-alpha.10
...
function getQuadrant(angle):
    ...
    // WIDER quadrant ranges v6.0.0-alpha.10
...
function calculatePositions(root, canvas):
    ...
    num_l1 = count(root.children)
    // Start angle offset to avoid placing first node at exact top
    ...
    for each level1_child at index i:
        ...
        if num_l2 > 0:
            // WIDER spread for Level 2: ±85° (was ±80°) v6.0.0-alpha.11
            spreadAngle = 170 / (num_l2 + 1)
            for each level2_child at index j:
                child_angle = level1_child.angle - 85 + spreadAngle * (j + 1)
                child_angle_rad = child_angle * PI / 180
                
                // Level 2 radius: 280px
                ...
                level2_child.width = len(level2_child.label) > 15 ? 160 : 140
                
                // MANDATORY L2 STAGGER (ENHANCED v6.0.0-alpha.11) - ±60px (was ±50px)
                if j % 2 == 1:
                    level2_child.y += 60
                else if j % 3 == 2:
                    level2_child.y -= 60
                ...
                if num_l3 > 0:
                    // WIDER fan spread for Level 3: ±75° (was ±70°) v6.0.0-alpha.11
                    l3_spread = 150 / (num_l3 + 1)
                    for each level3_child at index k:
                        l3_angle = level2_child.angle - 75 + l3_spread * (k + 1)
                        l3_angle_rad = l3_angle * PI / 180
                        
                        // Level 3 radius: 220px
                        level3_child.x = level2_child.x + 220 * cos(l3_angle_rad)
                        level3_child.y = level2_child.y + 220 * sin(l3_angle_rad)
                        
                        // Dynamic node width based on text length
                        level3_child.width = len(level3_child.label) > 15 ? 140 : 120
                        
                        // STAGGER vertically by ±70px (was ±60px) v6.0.0-alpha.11
                        if k % 2 == 1:
                            level3_child.y += 70
                        else if k % 3 == 2:
                            level3_child.y -= 70
    
    // SIBLING L2 SUBTREE COLLISION CHECK (ENHANCED v6.0.0-alpha.11)
    for each level1_node in root.children:
        ...
        for i = 0 to len(l2_children) - 2:
            ...
            if boxesOverlap(box_a, box_b) or distance(box_a, box_b) < 250:
                // Increase L2 radius by 40% and rotate fan by 30° (was 35%/25°)
                l2_children[i].radius *= 1.40
                l2_children[i+1].radius *= 1.40
                l2_children[i+1].angle += 30
                recalculate_l2_positions(level1_node)
    
    // HORIZONTAL LINE DETECTION (ENHANCED v6.0.0-alpha.11 - CHECKS L2 AND L3)
    all_l2_l3_nodes = collect_all_l2_and_l3_nodes()
    for i = 0 to len(all_l2_l3_nodes) - 1:
        for j = i + 1 to len(all_l2_l3_nodes) - 1:
            // Tightened threshold: 50px (was 40px) and triggers on 2 nodes (not 3)
            if abs(all_l2_l3_nodes[i].y - all_l2_l3_nodes[j].y) < 50:
                // Stagger by 70px (was 60px)
                node_to_move = further_from_center(all_l2_l3_nodes[i], all_l2_l3_nodes[j])
                node_to_move.y += 70
    
    // COLLISION DETECTION PASS (ENHANCED v6.0.0-alpha.11)
        ...
        for each node_a in all_nodes:
            for each node_b in all_nodes where node_b != node_a:
                if not connected(node_a, node_b):
                    ...
                    min_distance = 120 + (node_a.width + node_b.width) / 2  // 120px gap (was 100px)
                    if distance < min_distance:
                        collision_found = true
                        // Increase radius of deeper node's parent by 40% (was 35%)
                        deeper_node = node_a.level > node_b.level ? node_a : node_b
                        deeper_node.parent.radius *= 1.40
                        recalculate_branch(deeper_node.parent)
        iteration++
    
    // MARGIN VALIDATION PASS
...
<layout_tips>
## Critical Layout Rules (MUST FOLLOW) - UPDATED v6.0.0-alpha.11
...
### Node Sizing (DYNAMIC - BASED ON TEXT LENGTH) - v6.0.0-alpha.11
- Root: 180×90
- Level 1: 160×70
- Level 2: **160×55 for long text (>15 chars), 140×55 for short text**
- Level 3+: **140×45 for long text (>15 chars), 120×45 for short text**
- **NEVER truncate text** - always size node to fit full label
...
### Anti-Cramping Rules (CRITICAL - v6.0.0-alpha.11)
- **Level 2 nodes MUST be staggered by ±60px vertically** (increased from ±50px!)
- **Level 3 nodes MUST be staggered by ±70px vertically** (increased from ±60px)
- **Minimum 120px gap** between non-connected nodes (increased from 100px)
- If children overlap, increase parent radius by **40%** (increased from 35%)
- **Run collision detection** after all positions calculated

### Fan Angles (MAXIMUM SPREAD - v6.0.0-alpha.11)
- **Level 2 fan: ±85°** from parent angle (increased from ±80°)
- **Level 3 fan: ±75°** from parent angle (increased from ±70°)
- Deeper levels: ±65° minimum

### Sibling L2 Collision Prevention (ENHANCED v6.0.0-alpha.11 - CRITICAL)
- **MANDATORY:** Apply ±60px vertical stagger to ALL L2 children BEFORE other positioning
- When L1 parent has multiple children, verify NO overlap between L2 subtrees (within 250px)
- Calculate L2 boundary box (L2 node + all its L3 children)
- If boundary boxes overlap: INCREASE L2 radius by 40% AND rotate fan by 30°
- **L2 children of same L1 must NEVER share similar Y-coordinates (within 50px)**

### Horizontal Line Prevention (ENHANCED v6.0.0-alpha.11 - APPLIES TO L2 AND L3!)
- After position calculation, scan ALL L2 AND L3 nodes (not just L3!)
- **Threshold reduced:** If 2+ nodes (not 3!) have y-coordinates within ±50px: STAGGER
- Stagger amount: **±70px vertical offset** (increased from 60px)
- Priority: Stagger the node furthest from canvas center
- This prevents horizontal bands at ANY level

### Quadrant-Aware Positioning (ENHANCED v6.0.0-alpha.11)
- **TOP branches (210°-330°):** Children fan UPWARD and outward with ±85° spread
- **BOTTOM branches (30°-150°):** Children fan DOWNWARD and outward with ±85° spread
- **LEFT branches (150°-210°):** Children fan LEFT and outward with ±85° spread
- **RIGHT branches (330°-30°):** Children fan RIGHT and outward with ±85° spread
- This prevents nodes from clustering toward the center of the diagram

### Collision Detection Algorithm (ENHANCED v6.0.0-alpha.11)
1. Calculate all node positions using radial algorithm
2. **First pass - L2 vertical stagger:**
   - Apply mandatory ±60px stagger to ALL L2 nodes immediately
3. **Second pass - Sibling L2 subtree collision:**
   - For each L1 parent with multiple L2 children
   - Calculate bounding box of each L2 + its L3 children
   - If boxes overlap or within 250px: increase L2 radius by 40% AND rotate by 30°
4. **Third pass - Horizontal line detection (L2 AND L3):**
   - Scan ALL L2 and L3 nodes
   - If 2+ nodes have y within 50px: stagger by 70px
5. **Fourth pass - Individual node collision:**
   - For each pair of non-connected nodes:
   - If distance < 120px + (node1.width + node2.width)/2: COLLISION
   - Increase parent radius by 40% and recalculate
6. For edge nodes (within 250px of canvas boundary):
   - Push node inward to maintain margin

### Common Mistakes to Avoid (UPDATED v6.0.0-alpha.11)
1. ❌ Using canvas smaller than 2400×1800 for 6+ branches
2. ❌ Fixed node width regardless of text length (USE DYNAMIC SIZING!)
3. ❌ Placing 2+ L2 OR L3 nodes in a near-horizontal line (STAGGER THEM - threshold is 2!)
...
7. ❌ Fan angles too narrow (USE ±85° for L2, ±75° for L3)
8. ❌ Skipping collision detection before output
9. ❌ Nodes within 250px of canvas edge
10. ❌ L2 sibling subtrees overlapping (CHECK BOUNDARY BOXES - 250px threshold)
11. ❌ Not staggering L2 nodes vertically (must stagger ±60px!)
12. ❌ Only checking L3 for horizontal lines (MUST CHECK L2 TOO!)
13. ❌ Truncating long text labels (INCREASE NODE WIDTH INSTEAD!)
</layout_tips>
