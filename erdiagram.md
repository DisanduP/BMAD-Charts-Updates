...
- Entities MUST be arranged in a grid pattern with generous spacing
- Minimum horizontal gap between entities: 300px
- Minimum vertical gap between entities: 250px
- Entity boxes should have consistent widths (150-200px)
- Entity height depends on attribute count: base 40px + 20px per attribute
...
- Relationship lines MUST NEVER pass through entity boxes
- Use orthogonal routing (right-angle turns only)
- Lines should exit/enter entities from the NEAREST side to minimize length
- For entities on same row: use horizontal connections
- For entities in same column: use vertical connections
- For diagonal relationships: use L-shaped or Z-shaped routing with waypoints
...
- Labels must have OFFSET to avoid overlapping with entity boxes
- For horizontal lines: offset label ABOVE the line (y offset: -15)
- For vertical lines: offset label to the LEFT of the line (x offset: -40)
...
- Cardinality symbols (crow's foot, etc.) must be placed 10-20px from entity border
...
- For ER diagrams, use GRID-BASED entity placement
- Minimum horizontal gap between entities: 300px
- Minimum vertical gap between entities: 250px
- Entity width: 180px consistent
- Entity height: 30px header + 22px per attribute
- Use swimlane shapes for entities with stackLayout for attributes
- Relationship lines MUST use orthogonal routing with explicit waypoints
- Cardinality symbols use Draw.io ER arrows: ERone, ERzeroToOne, ERoneToMany, ERzeroToMany
- Relationship labels must be offset from line midpoint to avoid entity overlap
- Route complex relationships AROUND entities, never through them
...
STRICTLY follow the ER Diagram Rules (Section 5) from the conversion-ruleset.
</instructions>
...
- CRITICAL: Relationship labels must be positioned at line midpoints with proper offsets
...
2. Calculate Entity Dimensions:
   - Width: 180px (consistent for all entities)
   - Height: 30px (header) + 22px per attribute
   - Add 10px padding top/bottom

3. Layout Entities Using Grid Algorithm:
   - Identify the entity with most relationships (center it)
   - Place directly connected entities around it
   - Use minimum spacing: 300px horizontal, 250px vertical
   - Arrange in logical tiers based on relationship proximity:
     * Tier 0 (center): Main/central entity
     * Tier 1: Directly connected to center
     * Tier 2: Connected to Tier 1 entities
   
4. Entity Positioning Strategy for Complex Diagrams:
   - Top row: Independent/reference entities (e.g., MOVIE, THEATER)
   - Middle row: Junction/relationship entities (e.g., SHOW, SEAT)
   - Bottom row: Transaction entities (e.g., BOOKING, TICKET, PAYMENT)
   - Customer/User entities: Far left or right of top row

5. Route Relationships (CRITICAL - prevent line overlap):
   a) Determine optimal exit/entry sides:
      - Same row entities: exit RIGHT (1, 0.5) → enter LEFT (0, 0.5)
      - Same column entities: exit BOTTOM (0.5, 1) → enter TOP (0.5, 0)
      - Diagonal: Use closest sides with waypoints for turns
   
   b) Edge style for ER diagrams:
      - Base: "edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;"
      - Add startArrow and endArrow based on cardinality
      - Cardinality symbols:
        * || (exactly one): startArrow=ERone;startFill=0
        * |o (zero or one): startArrow=ERzeroToOne;startFill=0
        * }| or |{ (one or many): startArrow=ERoneToMany;startFill=0
        * }o or o{ (zero or many): startArrow=ERzeroToMany;startFill=0
   
   c) Add waypoints for complex routes:
      - When line would cross another entity, add mxPoint waypoints
      - Route around entities with 50px clearance minimum

6. Position Relationship Labels:
   - Place at edge midpoint (relative="1" with x="0.5" internally)
   - Offset AWAY from any nearby entity:
     * Horizontal lines: y="-15" (above line)
     * Vertical lines: x="-50" (left of line)
   - Use fontSize=11 for relationship labels
   - Format: "relationship_name (cardinality)" e.g., "makes (1:N)"

7. Assemble XML with ER-specific structure
</process>
  ...
  <diagram name="[ER Diagram Name]" id="[unique]">
    <mxGraphModel dx="1200" dy="800" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="1100" pageHeight="850" math="0" shadow="0">
      <root>
        <mxCell id="0"/>
        <mxCell id="1" parent="0"/>
        [Entity containers as swimlane mxCells]
        [Attribute rows as child mxCells]
        [Relationship edges with proper routing and labels]
      </root>
    </mxGraphModel>
  </diagram>
</mxfile>
...
For complex ER diagrams with 6+ entities (like movie ticket booking):

LAYOUT GRID (recommended positions):
```
Row 1 (y=40):   [CUSTOMER x=40]     [MOVIE x=340]      [THEATER x=640]
Row 2 (y=320):  [BOOKING x=40]      [SHOW x=340]       [SEAT x=640]  
Row 3 (y=600):  [PAYMENT x=40]      [TICKET x=340]
```

RELATIONSHIP ROUTING:
- CUSTOMER → BOOKING: Exit bottom (0.5,1), enter top (0.5,0) - vertical, no conflict
- MOVIE → SHOW: Exit bottom (0.5,1), enter top (0.5,0) - vertical, no conflict  
- THEATER → SHOW: Exit bottom left (0.25,1), enter top right (0.75,0) - slight diagonal
- THEATER → SEAT: Exit bottom (0.5,1), enter top (0.5,0) - vertical, no conflict
- BOOKING → TICKET: Exit bottom (0.5,1), enter top (0.5,0) - vertical, no conflict
- BOOKING → PAYMENT: Exit right (1,0.5), enter left (0,0.5) then down - use waypoints
- SHOW → TICKET: Exit bottom (0.5,1), enter top (0.5,0) - vertical, no conflict
- SEAT → TICKET: Exit bottom left (0.25,1), enter top right (0.75,0) - use waypoints to avoid SHOW

LABEL POSITIONING:
- Vertical relationships: Label offset x=-50 (to the left of line)
- Horizontal relationships: Label offset y=-15 (above line)
- Diagonal relationships: Use waypoints, label on longest segment
</complex_er_layout_strategy>
